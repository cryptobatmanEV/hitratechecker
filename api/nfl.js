...
async function fetchSeasonItems(athleteId, season) {
  ...
  const addItems = (list) => {
    for (const i of list) {
      ...
      const key = i.event?.$ref || i.statistics?.$ref || '';
      if (key && seen.has(key)) continue;
      if (key) seen.add(key);
      // Skip preseason (seasonType id=1) but include regular season + postseason
      if (i.seasonType?.id === 1) continue;
      items.push(i);
    }
  };

  // Pass 1: generic eventlog (returns regular season reliably)
  let page = 1;
  while (true) {
    try {
      const data = await espnGet(`${BASE}?limit=25&page=${page}`);
      addItems(data.events?.items || []);
      if (page >= (data.events?.pageCount || 1)) break;
      page++;
    } catch { break; }
  }

  // Pass 2: postseason supplement — try seasonType=3 query param
  try {
    const data = await espnGet(`${BASE}?limit=25&seasonType=3`);
    addItems(data.events?.items || []);
  } catch {}
