...
async function fetchSeasonItems(athleteId, season) {
  ...
  let page = 1;
  while (true) {
    const url = `https://sports.core.api.espn.com/v2/sports/${SPORT}/leagues/${LEAGUE}/seasons/${season}/athletes/${athleteId}/eventlog?limit=25&page=${page}`;
    const data = await espnGet(url);
    // Skip preseason (seasonType 1)
    // Include all played events (regular season + playoffs); preseason has minimal stats anyway
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'gamelog') {
      ...
      // Fetch 3 NFL seasons + current calendar year (Super Bowl is often in Feb = next calendar year)
      const calYear = new Date().getFullYear();
      const extraYears = calYear !== curSeason ? [calYear] : [];
      const seasons = [...new Set([...extraYears, curSeason, curSeason - 1, curSeason - 2])];
