...
async function fetchWithTimeout(url, options = {}, ms = 9000) {
  ...
  try {
    ...
    clearTimeout(timer);
    if (e.name === 'AbortError') throw new Error('NBA API timed out after 9s');
    throw e;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    if (action === 'search') {
      ...
      const r = await fetchWithTimeout(url, { headers: NBA_HEADERS });
      const text = await r.text();
      if (!r.ok) return res.status(500).json({ error: `NBA returned ${r.status}: ${text.slice(0,200)}` });
      let d;
      try { d = JSON.parse(text); } catch(e) { return res.status(500).json({ error: `NBA returned non-JSON: ${text.slice(0,200)}` }); }
