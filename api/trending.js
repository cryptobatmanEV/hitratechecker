...
async function fetchCS2BatchLogs(ppPlayerNames) {
  // GRID schema confirmed:
  // - allSeries uses edges.node (top-level connection)
  // - Series.players uses nodes (nested relationship)
  // - Date field is startTimeScheduled
  ...
  for (const s of allNodes.filter(s=>s?.type==='ESPORTS')) {
    const date = (s.startTimeScheduled||s.id||'').slice(0,10);
    for (const p of (s.players?.nodes||[])) {
      if (!p?.nickname) continue;
      const n = norm(p.nickname);
      if (!logsByNick[n]) logsByNick[n] = [];
      logsByNick[n].push({ kills: p.kills||0, _date: date });
    }
  }

  const result = {};
  for (const name of ppPlayerNames) {
    result[name] = logsByNick[norm(name)] || [];
  }
  return result;
}
