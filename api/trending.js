...
async function findPlayer(name, sport, host) {
  try {
    ...
    if (sport === 'dota') {
      // OpenDota search directly — /api/dota search takes 12s+ (too slow for trending)
      const r = await fetch(`https://api.opendota.com/api/search?q=${encodeURIComponent(name)}`).catch(()=>null);
      if (!r?.ok) return null;
      const d = await r.json().catch(()=>[]);
      if (!Array.isArray(d)||!d.length) return null;
      const match = d.find(p=>norm(p.personaname||'')===n)||d[0];
      return match ? { id:String(match.account_id), name:match.personaname||name } : null;
    }
...
async function fetchLog(player, sport, host) {
  try {
    ...
    if (sport === 'dota') {
      // OpenDota recentMatches — single fast call, no API key needed
      const r = await fetch(`https://api.opendota.com/api/players/${player.id}/recentMatches?limit=60`);
      if (!r.ok) return [];
      const all = await r.json();
      if (!Array.isArray(all)) return [];
      // Keep competitive matches (lobby_type=1)
      const comp = all.filter(m=>m.lobby_type===1);
      // Group into series: matches within 4 hours of each other
      const series = [];
      let i = 0;
      while (i < comp.length) {
        const cur = comp[i];
        const group = [cur];
        while (i+group.length < comp.length) {
          const next = comp[i+group.length];
