...
async function fetchCS2BatchLogs(ppPlayerNames) {
  // Step 1: search GRID Central Data for each PP player → get teamId
  const playerMeta = {};
  await Promise.all(ppPlayerNames.map(async name => {
    ...
    for (const f of [`equals:"${safe}"`,`contains:"${safe}"`]) {
      ...
      const cs2p = all.find(p=>p.title?.id==='28')||all[0];
      if (cs2p?.team?.id) {
  ...
  // Step 2: for each found player, get recent series IDs from Central Data
  const oneYearAgo = new Date(Date.now()-365*86400000).toISOString();
  const playerSeries = {};
  await Promise.all(Object.entries(playerMeta).map(async ([name,meta]) => {
    const d = await cdQ(`{allSeries(filter:{teamIds:{in:["${meta.teamId}"]},startTimeScheduled:{gte:"${oneYearAgo}"}},first:20,orderBy:StartTimeScheduled){edges{node{id startTimeScheduled}}}}`);
    const ids = (d?.data?.allSeries?.edges||[]).map(e=>e.node?.id).filter(Boolean);
    playerSeries[name] = { ...meta, ids: ids.slice(0,15) };
  }));

  // Step 3: batch query Live Data Feed for kills via seriesState
  const SP = `id name kills killAssistsGiven`;
  const logMap = {};

  await Promise.all(Object.entries(playerSeries).map(async ([name,meta]) => {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const projs = (ppData.data||[])
      ...
      .map(p=>{
        ...
        return {
          ...
          allowOver:  odds!=='goblin',   // goblin = under only on PP
          allowUnder: odds!=='demon',    // demon = over only on PP
          isCombo:    combo,
        };
      })
    ...
    const SCOPE_PATTERNS = {
      'maps1-2': /^MAPS\s+1-2/i,
      'maps1-3': /^MAPS\s+1-3/i,
      'map1': /^MAP\s+1/i,
      'map2': /^MAP\s+2/i,
      'map3': /^MAP\s+3/i,
      'map4': /^MAP\s+4/i,
