...
function seasonStr(offset = 0) {
  ...
  // WNBA season runs May-September, so current season starts in May
...
export default async function handler(req, res) {
  ...
  try {
    if (action === 'search') {
      ...
      if (!r.ok) throw new Error(`WNBA API returned ${r.status}`);
    ...
    if (action === 'gamelog') {
      const seasons = scope === 'career'
        ? [seasonStr(0), seasonStr(1), seasonStr(2), seasonStr(3)]
        : [seasonStr(0)];
