...
const PP_LEAGUE = { nba:'7', wnba:'3', nfl:'9', cfb:'15', mlb:'2', nhl:'8', lol:'121', dota:'174', cs2:'265', tennis:'5' };
...
const CALCS = {
  ...
  // Tennis: stats derived from score string — no aces/DFs available from free API
  tennis: {
    'Total Sets':       g => g.totalSets,
    'Total Games':      g => g.totalGames,
    'Total Games Won':  g => g.totalGamesWon,
    'Total Tie Breaks': g => g.totalTieBreaks,
  },
};
...
async function findPlayer(name, sport, host) {
  try {
    const n = norm(name);
    if (sport === 'tennis') {
      const r = await fetch(`https://${host}/api/tennis?action=search&q=${encodeURIComponent(name)}`);
      if (!r.ok) return null;
      const d = await r.json();
      return d?.id ? d : null;
    }
