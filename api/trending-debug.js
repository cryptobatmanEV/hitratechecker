...
export default async function handler(req, res) {
  ...
  if (sport === 'cs2') {
    ...
    // Step 1: introspect Series to find correct field names
    const intro = await gp(`query { __type(name:"Series") { fields { name type { name kind } } } }`);
    out.series_fields = intro.data?.__type?.fields?.map(f=>f.name);

    // Step 2: test correct path - Series → teams → players → kills (no games, no startTime)
    const result = await gp(`
      query { allSeries(first:5) {
        edges { node {
          id type
          teams { edges { node {
            name
    ...
    out.sample = nodes.slice(0,2).map(s=>({
      id: s.id, type: s.type,
      teams: (s.teams?.edges||[]).map(te=>({
        name: te.node?.name,
        player_count: te.node?.players?.edges?.length,
        players: (te.node?.players?.edges||[]).slice(0,3).map(pe=>({
          nickname: pe.node?.nickname,
          kills: pe.node?.kills,
        })),
      })),
    }));
