...
const gameCache = new Map(); // in-memory fallback

// Vercel KV persistent cache — survives container restarts, 24hr TTL
...
async function kvSet(key, value) {
  ...
  // Upstash Redis REST API pipeline format
  try {
    const encoded = encodeURIComponent(JSON.stringify(value));
    await fetch(`${KV_URL}/pipeline`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${KV_TOKEN}`, 'Content-Type': 'application/json' },
      body: JSON.stringify([['SET', key, encoded, 'EX', '86400']]),
      signal: AbortSignal.timeout(2000), // 2s max — never block main response
    });
  } catch {}
}
...
// Verified HLTV IDs — skips JS search (5 credits) for common DFS players
const VERIFIED_IDS = {
  ...
  'woxic':      { id: '11378', slug: 'woxic' },
  'xantares':   { id: '11378', slug: 'XANTARES' },
  'jks':        { id: '11393', slug: 'jks' },
  'hooxi':      { id: '9345',  slug: 'HooXi' },
};
...
function parseHLTVDate(s) {
  // HLTV format: DD/MM/YY → convert to Date
...
export default async function handler(req, res) {
  ...
  try {
      ...
      if (slugMatches.length) {
        return res.json({
          players: slugMatches.map(slug => {
            ...
            return {
              id:   v ? `hltv_${v.id}_${v.slug}` : `hltv_search_${slug}`,
              name: slug,
    ...
    // ── Game log ─────────────────────────────────────────────────────────────
    if (action === 'gamelog') {
      // Check KV cache first (persists 24hr across all containers), then in-memory
      const cached = await kvGet(playerId) || (gameCache.has(playerId) ? gameCache.get(playerId) : null);
      if (cached) return res.json({ games: cached, cached: true });
