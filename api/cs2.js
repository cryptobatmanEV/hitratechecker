...
export default async function handler(req, res) {
  ...
  try {
    // ── Player search by FACEIT nickname ──────────────────────────────────────
    if (action === 'search') {
      const d = await ff(`/players?nickname=${encodeURIComponent(nickname)}&game=cs2`);
      return res.json({
        players: [{
          id: d.player_id,
          name: d.nickname,
          sub: `Level ${d.games?.cs2?.skill_level||'?'} · ${d.country||''}`.trim()
        }]
      });
    }
        ...
        try {
          ...
          for (const round of rounds) {
            for (const team of round.teams || []) {
              ...
              const oppTeam = (round.teams || []).find(t =>
                !t.players?.some(x => x.player_id === playerId)
              );
              ...
              games.push({
                ...
                _date:     match.started_at
                  ? new Date(match.started_at*1000).toISOString().split('T')[0]
                  : '',
                _opp:      oppTeam?.team_stats?.['Team']||'',
              });
            }
          }
        } catch(e) { continue; }
