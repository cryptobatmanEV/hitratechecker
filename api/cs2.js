...
export default async function handler(req, res) {
  ...
  try {
    // ── Search — Level 10 only, sorted by ELO desc ────────────────────────
    if (action === 'search') {
      const d = await ff(`/search/players?nickname=${encodeURIComponent(nickname)}&game=cs2&offset=0&limit=20`);
      const items = (d.items || [])
        .filter(p => p.games?.cs2?.skill_level === 10)          // Level 10 only
        .sort((a, b) => (b.games?.cs2?.faceit_elo || 0) - (a.games?.cs2?.faceit_elo || 0))
        .slice(0, 8);
      ...
      return res.json({
        players: items.map(p => ({
          id: p.player_id,
          name: p.nickname,
          sub: `ELO ${p.games?.cs2?.faceit_elo || '?'} · ${p.country?.toUpperCase() || ''}`.trim()
        }))
      });
    }
