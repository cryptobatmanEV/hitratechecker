...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'gamelog') {
      ...
      const cacheKey = `grid_v2_${statsId}_${today}`;
      ...
      // Step 2: metadata in 2 batches of 15 (GRID complexity limit ~15 per query)
      const slice = ids.slice(0, 30);
      const meta  = [];
      for (let i = 0; i < slice.length; i += 15) {
        const chunk  = slice.slice(i, i + 15);
        const fields = chunk.map((id, j) =>
          `s${i+j}: series(id:"${id}") { id startTimeScheduled tournament { id } teams { baseInfo { id name } } }`
        ).join('\n');
        const md = await cdQ(`{ ${fields} }`);
        meta.push(...Object.values(md?.data || {}).filter(Boolean));
      }
