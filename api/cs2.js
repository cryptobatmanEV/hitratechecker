...
export default async function handler(req, res) {
  ...
  try {
    if (action === 'search') {
      ...
      const items = (d.items || [])
        .filter(p => parseInt(p.games?.cs2?.skill_level) === 10)   // FIX: parseInt, not strict ===
        .sort((a, b) => (parseInt(b.games?.cs2?.faceit_elo) || 0) - (parseInt(a.games?.cs2?.faceit_elo) || 0))
        .slice(0, 8);

      // If no Level 10 results, fall back to showing top results regardless of level
      const fallback = items.length === 0
        ? (d.items || []).sort((a,b)=>(parseInt(b.games?.cs2?.faceit_elo)||0)-(parseInt(a.games?.cs2?.faceit_elo)||0)).slice(0,5)
        : items;
      ...
      return res.json({
        players: fallback.map(p => ({
          id: p.player_id,
          name: p.nickname,
          sub: `Level ${p.games?.cs2?.skill_level || '?'} · ELO ${p.games?.cs2?.faceit_elo || '?'} · ${(p.country||'').toUpperCase()}`
        }))
      });
    }
