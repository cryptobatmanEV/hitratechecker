...
async function hltvSearch(query) {
  ...
  const rx = /href="\/player\/(\d+)\/([^"?#]+)"/gi; let m;
...
async function resolveHLTVPlayer(slug) {
  // Check KV cache first — 7 day TTL so we rarely pay for HLTV search
  ...
  // Cache for 7 days — player HLTV IDs never change
  if (KV_URL) {
    try {
      await fetch(`${KV_URL}/setex/${encodeURIComponent(idKey)}/604800/${encodeURIComponent(JSON.stringify(results[0]))}`, {
