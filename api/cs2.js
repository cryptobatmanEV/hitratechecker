...
const GRID_CD   = 'https://api-op.grid.gg/central-data/graphql';
const GRID_STATS = 'https://api-op.grid.gg/statistics-feed/graphql';

async function gridCD(query) {
  const r = await fetch(GRID_CD, { method:'POST', headers:{'Content-Type':'application/json','x-api-key':GRID_KEY}, body:JSON.stringify({query}) });
  return r.json();
}
async function gridStats(query) {
  const r = await fetch(GRID_STATS, { method:'POST', headers:{'Content-Type':'application/json','x-api-key':GRID_KEY}, body:JSON.stringify({query}) });
  return r.json();
}

// Get full game log from GRID using player nickname
async function getGridGameLog(nickname) {
  if (!GRID_KEY) return null;
  try {
    // Find player profiles
    const pd = await gridCD(`{ players(filter:{nickname:{equals:"${nickname.replace(/"/g,'')}"}},first:5){edges{node{id nickname title{id}team{id name}}}} }`);
    const players = pd?.data?.players?.edges?.map(e=>e.node)||[];
    const csgo = players.find(p=>p.title?.id==='1');
    const cs2  = players.find(p=>p.title?.id==='28');
    const statsId = csgo?.id||cs2?.id;
    const teamId  = cs2?.team?.id||csgo?.team?.id;
    if (!statsId||!teamId) return null;

    // Get overall stats + confirmed series IDs
    const sd = await gridStats(`{ playerStatistics(playerId:"${statsId}",filter:{timeWindow:LAST_6_MONTHS}){ aggregationSeriesIds series{ count kills{sum} deaths{sum} won{value count} ...on CsgoPlayerSeriesStatistics{headshots{sum}} } } }`);
    const pStats = sd?.data?.playerStatistics;
    const seriesIds = pStats?.aggregationSeriesIds||[];
    if (!seriesIds.length) return null;

    const totalCount  = pStats.series?.count||1;
    const totalKills  = pStats.series?.kills?.sum||0;
    const totalDeaths = pStats.series?.deaths?.sum||0;
    const totalHS     = pStats.series?.headshots?.sum||0;
    const totalWins   = pStats.series?.won?.find(w=>w.value===true)?.count||0;

    // Get series metadata (dates, opponents) in batches of 10
    const batches = [];
    for (let i=0;i<seriesIds.slice(0,30).length;i+=10) {
      const ids = seriesIds.slice(i,i+10);
      const q = ids.map((id,j)=>`s${j}:series(id:"${id}"){id startTimeScheduled tournament{id}teams{baseInfo{id name}}}`).join('
');
      const md = await gridCD(`{${q}}`);
      batches.push(...Object.values(md?.data||{}).filter(Boolean));
    }

    // Group series by tournament for per-tournament stats
    const tournMap = {};
    for (const s of batches) { const tid=s.tournament?.id; if(tid){if(!tournMap[tid])tournMap[tid]=[];tournMap[tid].push(s);} }
    const tournIds = Object.keys(tournMap).slice(0,10);

    // Query per-tournament stats in parallel
    const tournStats = {};
    await Promise.all(tournIds.map(async tid=>{
      try {
        const d = await gridStats(`{ playerStatistics(playerId:"${statsId}",filter:{tournamentIds:{in:["${tid}"]}}){ aggregationSeriesIds series{count kills{sum}deaths{sum}won{value count}...on CsgoPlayerSeriesStatistics{headshots{sum}}} } }`);
        const ts = d?.data?.playerStatistics;
        if (ts?.series?.count>0) tournStats[tid]=ts;
      } catch{}
    }));

    // Build game log
    const games = batches.map(s=>{
      const opp = s.teams?.find(t=>t.baseInfo?.id!==teamId)?.baseInfo?.name||'?';
      const tid = s.tournament?.id;
      const ts  = tournStats[tid];
      ...
      return { kills,deaths,assists:0,headshots,win,maps:[],_date:s.startTimeScheduled?.split('T')[0]||'',_opp:opp,_matchUrl:null };
    });
...
function parseHLTVMatches(html) {
  ...
  while ((rowM = rowRx.exec(tHTML)) !== null) {
    ...
    const mUrlM = rowHTML.match(/href="(\/stats\/matches\/mapstatsid\/[^"?]+)/);
    games.push({ kills, deaths, assists: 0, headshots: 0, win, map: cells[3] || '', _date: cells[0] || '', _opp: opp, _matchUrl: mUrlM ? mUrlM[1] : null });
  }
...
function groupIntoSeries(maps) {
  ...
  while (i < maps.length) {
    ...
    series.push({
      ...
      maps:      [...group].reverse().map(g => ({ kills: g.kills, deaths: g.deaths, assists: 0, headshots: g.headshots || 0, map: g.map })),
...
// Fetch headshots for a single map — searches both team tables for the player
async function fetchMatchHeadshots(matchUrl, playerSlug) {
  try {
    const html = await scraperFetch(`https://www.hltv.org${matchUrl}`, false);
    ...
    while ((tableM = tableRx.exec(html)) !== null) {
      ...
      while ((rowM = rowRx.exec(tableM[1])) !== null) {
        const row = rowM[1];
        const pCell = row.match(/class="[^"]*st-player[^"]*"[^>]*>([\s\S]*?)<\/td>/i)?.[1] || '';
        if (!pCell.toLowerCase().includes(playerSlug.toLowerCase())) continue;
        const killsCell = row.match(/class="st-kills[^"]*traditional[^"]*"[^>]*>([\s\S]*?)<\/td>/i)?.[1] || '';
        const hsMatch = killsCell.match(/\((\d+)\)/);
        return hsMatch ? parseInt(hsMatch[1]) : 0;
      }
    }
  } catch {}
  return 0;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'gamelog') {
      ...
      const isHS = req.query.stat === 'headshots';
      const cacheKey = `${playerId}_${today}${isHS ? '_hs' : ''}`;
      ...
      // Try GRID first (free, no credits, includes headshots)
      if (GRID_KEY && playerId?.startsWith('hltv_')) {
        const parts = (playerId||'').split('_');
        const slug = parts.slice(2).join('_');
        const gridGames = await getGridGameLog(slug);
        if (gridGames?.length) {
          gameCache.set(cacheKey, gridGames);
          kvSet(cacheKey, gridGames);
          return res.json({ games: gridGames });
        }
      }
      ...
      if (playerId?.startsWith('hltv_')) {
        ...
        if (isHS) {
          try {
            const mapsForHS = rawMaps.filter(m => m._matchUrl).slice(0, 20);
