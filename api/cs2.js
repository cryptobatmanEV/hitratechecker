...
// Uses header text to find column indices, applies to data rows positionally
function parseHLTVMatches(html) {
  // Find the specific stats-matches-table
  const tMatch = html.match(/<table[^>]*stats-matches-table[^>]*>([\s\S]*?)<\/table>/i);
  if (!tMatch) return { games: [], debug: 'table not found in HTML' };
  const tHTML = tMatch[1];

  // Read all <th> header texts
  const headers = [];
  ...
  // HLTV table: Date|Team1(desktop)|Team2(desktop)|T1(mobile)|T2(mobile)|Map|Result|K|D|+/-|Rating
  let ki  = headers.findIndex(h => h === 'k');
  let di  = headers.findIndex(h => h === 'd');
  let dti = headers.findIndex(h => h === 'date');
  let mi  = headers.findIndex(h => h === 'map');
  let ri  = headers.findIndex(h => h === 'result' || h === 'res');
  let oi  = headers.findIndex(h => h === 'opponent' || h === 'opp');

  // Positional fallback if header text detection fails
  if (ki  === -1) ki  = 7;
  if (di  === -1) di  = 8;
  if (dti === -1) dti = 0;
  if (mi  === -1) mi  = 5;
  ...
  while ((rowM = rowRx.exec(tHTML)) !== null) {
    ...
    if (cells.length < Math.max(ki, di) + 1) continue;
    ...
    games.push({
      ...
      win:   ri < cells.length ? cells[ri].toLowerCase().startsWith('w') : null,
      map:   mi < cells.length ? cells[mi] : '',
      _date: dti < cells.length ? cells[dti] : '',
      _opp:  oi < cells.length ? cells[oi] : '',
    });
  }

  return { games, headers, indices: { ki, di, dti, mi, ri, oi } };
}
