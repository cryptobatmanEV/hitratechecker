...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'gamelog') {
      ...
      // Step 1: Get series IDs from Statistics Feed
      const sf = await stQ(`{
        playerStatistics(playerId:"${gridId}", filter:{timeWindow:LAST_YEAR}) {
          aggregationSeriesIds
        }
      }`);
      const ids = (sf?.data?.playerStatistics?.aggregationSeriesIds || []).slice(0, 15);
