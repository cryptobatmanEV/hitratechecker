...
async function getGamelog(id, league) {
  ...
  const matches = comps.map((comp, i) => {
    ...
    let playerSetWins = 0, oppSetWins = 0;
    ...
    for (let s = 0; s < pSets.length; s++) {
      ...
      if (pg > og) playerSetWins++; else if (og > pg) oppSetWins++;
    }

    // Winner's set count determines format: 3 wins = BO5, 2 wins = BO3
    const matchWinnerSets = (me?.winner === true) ? playerSetWins : oppSetWins;
    const bestOf = matchWinnerSets === 3 ? 5 : 3;
  ...
  // Sort most-recent first — ESPN eventlog order is not guaranteed chronological
  matches.sort((a, b) => b._date.localeCompare(a._date));
